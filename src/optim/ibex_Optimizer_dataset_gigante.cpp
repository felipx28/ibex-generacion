...
Optimizer::Status Optimizer::optimize() {
	...
	try {
		...
		std::ofstream InputFile("/home/felipe/Documents/magister/model2/input/prueba_nuevo_dataset/input_dataset_gigante.txt", std::ios::app);
		std::ofstream OutputFile("/home/felipe/Documents/magister/model2/output/prueba_nuevo_dataset/output_dataset_gigante.txt", std::ios::app);
		...
		for (int k = 0 ; k < num_sim ; k++){
			...
			for (size_t i = 0; i < n; i++) {
				search_space += aux.front()->box[i].diam();
			}


			InputFile << "variables: " << variables << endl;
			InputFile << "restricciones: " << restricciones << endl;
			InputFile << "lb_f_obj: " << lb_f_obj << endl;
			InputFile << "ub_f_obj: " << ub_f_obj << endl;
			InputFile << "bigger_diam: " << bigger_diam << endl;
			InputFile << "lower_diam: " << lower_diam << endl;
			InputFile << "search_space: " << search_space << endl;
